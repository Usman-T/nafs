...
import { Challenge, CompletedTask, DailyTask, UserChallenge } from "@prisma/client";
...
const Challenges = ({ challenge, tasks }: ChallengesProps) => {
  ...
  const currentDay = Math.min(
    Math.floor((new Date().getTime() - new Date(challenge.startDate).getTime()) / (1000 * 3600 * 24) + 1),
                  ...
                  >
                    <div className="flex items-center">
                      <div
                        ...
                        style={{ backgroundColor: dailyTask.task.dimension.color }}
                      >
                        <IconComponent className="h-5 w-5" />
                      </div>
                      <span className={`text-[#ebdbb2] ${isCompleted ? "line-through opacity-70" : ""}`}>
                        {dailyTask.task.name}
                      </span>
                    </div>
                    <Link href={`/dashboard/challenges/complete/${dailyTask.id}`}>
                      <Button
                        variant="outline"
                        size="sm"
          ...
          <div className="w-full">
            ...
            <div className="flex space-x-1">
              {Array.from({ length: 7 }).map((_, i) => (
                <div
                  key={i}
                  className={`h-2 flex-1 rounded-full ${i < 5 ? "bg-[#fe8019]" : "bg-[#3c3836]"}`}
                ></div>
              ))}
            </div>
          </div>
